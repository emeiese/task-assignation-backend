...
def resolve_problem(
    ...
    Args:
        ...
        costs ([Dict[Dict[int]]]): Dictionary with the costs for each task for
            each human. Each key is a human name, while each subkey is a task.
        min_assign_task ([int]): Indicates the minimum number of assignations
            that a person must have on a task on a week or at the end of the
            periods.
        max_assign_task ([int|boolean]): Indicates the maximum number of
            assignations that a person can have on a task on a week or at the
            end of the periods. If false, it means the user didn't specify
            this variable.
        max_total_assign ([int|boolean]): Indicates the maximum number of total
            assignations that a person can have on a week or at the end of the
            periods. If false, it means the user didn't specify this variable.
        min_total_assign ([int]): Indicates the minimum number of total
            assignations that a person must have on a week or at the end
            of the periods.

    Returns:
        [Dict]: Returns a dict with the optimum assignation, the status of the
            problem and the value of the optimization function.
    """
    ...
    # No puede haber 1 persona en dos tareas el mismo día (si es que el número
    # de tareas es menor al de personas).
    if len(tasks) <= len(people):
        for person in people:
            for day in days:
                prob += lpSum([assign[
                    day, person, task] for task in tasks]) <= 1

    # Todos los días, cada turno debe estar asignado a 1 person (esto implica
    # implícitamente que habrán m turnos asignados por día).
    for day in days:
        for task in tasks:
            prob += lpSum([assign[
                day, person, task] for person in people]) == 1
    ...
    for task in tasks:
        for person in people:
            prob += (
                lpSum([assign[
                    day, person, task] for day in days]) >= min_assign_task
            )
    ...
    for person in people:
        for task in tasks:
            prob += (
                lpSum([assign[day, person, task] for day in days])
                <= temp_max_assign_task
            )
    ...
    for person in people:
        prob += (
            lpSum([assign[
                day, person, task] for (day, task) in day_task_tuples])
            >= min_total_assign
        )
